...
const EventAddModal = ({ isOpen, onClose, onSubmit, title }) => {
  ...
  useEffect(() => {
    ...
    async function loadLookups() {
      ...
      try {
        const [etRes, tgRes, orgRes, divRes, teamRes] = await Promise.all([
          ...
          fetch("/frontapi/employee/volunteer-team"),
        ]);
        ...
        const teamJson = teamRes.ok ? await teamRes.json().catch(() => null) : null;

        if (!mounted) return;

        setEventTypes(Array.isArray(etJson?.data) ? etJson.data : FALLBACK_EVENT_TYPES);
        setTargetGroups(Array.isArray(tgJson?.data) ? tgJson.data : FALLBACK_TARGET_GROUPS);
        setOrganizers(Array.isArray(orgJson?.data) ? orgJson.data : FALLBACK_ORGANIZERS);
        ...
        // setVolunteerTeams(Array.isArray(teamJson?.data) ? teamJson.data : (Array.isArray(teamJson) ? teamJson : []));
      } catch (err) {
