...
const ViewTable = ({ data, permissions }) => {
  ...
  const handleAddLocation = async (jsonData) => {
    setIsLoading(true);
    try {
      const res = await fetch(`/frontapi/employee/role`, {
        method: "POST",
        headers: {
  ...
  const handleEditLocation = async (jsonData) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`/frontapi/employee/role?id=${jsonData.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteLocation = async (id) => {
    setIsLoading(true);
    try {
      const res = await fetch(`/frontapi/employee/role?id=${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="border border-stroke bg-white px-6 pt-6 pb-4 rounded-lg shadow-md dark:border-strokedark dark:bg-boxdark">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <PermissionGate permissions={['add-roles']}>
          <Button
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-100 dark:bg-gray-800 text-left text-sm font-semibold">
              <th className="p-4 rounded-tl-lg">Role</th>
                ...
                >
                  ...
                  <td className="p-4">
                    <div className="flex flex-wrap gap-2">
                      {location.permissions?.length > 0 ? (
                        location.permissions.map((perm) => (
                          <span
                            key={perm.id}
                            className="bg-blue-100 text-blue-800 text-xs font-medium px-2 py-1 rounded dark:bg-blue-800 dark:text-blue-100"
                          >
                            {perm.name}
                          </span>
                        ))
