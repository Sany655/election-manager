...
export default function EditCandidatePage({ params }) {
  ...
  const fetchCandidate = async () => {
    try {
      const res = await fetch(`/frontapi/candidates/${id}`);
      if (res.ok) {
        ...
        setFormData({
          ...
          membership_number: candidate.membership_number || "",
          organization: candidate.organization || "",
          election_position: candidate.election_position || "",
          educational_background: candidate.educational_background || "",
          professional_experience: candidate.professional_experience || "",
          biography: candidate.biography || "",
          achievements: candidate.achievements || "",
          vision_mission: candidate.vision_mission || "",
          election_manifesto: candidate.election_manifesto || "",
          contact_information: candidate.contact_information || "",
          photo: null
        });
        if (candidate.photo) {
          setPhotoPreview(`${process.env.NEXT_PUBLIC_BASE_URL}/${candidate.photo}`);
        }
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(errorData.msg || "Failed to update candidate");
      }
    } catch (error) {
      console.error(error);
  ...
  if (loading) {
    return (
      <DefaultLayout title="Edit Candidate">
        <Loader />
      </DefaultLayout>
    );
  }
          ...
          <div className="mb-6 flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                <FaUserTie className="text-blue-600" /> Edit Candidate
              </h1>
              <p className="text-sm text-gray-500">Update the candidate details below.</p>
            </div>
          </div>
            ...
            <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="w-full md:w-3/4 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="text" name="msisdn" required value={formData.msisdn} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Password (Leave blank to keep current)</label>
                  <input type="password" name="password" value={formData.password} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="New password" />
                </div>
              </div>
            </div>
