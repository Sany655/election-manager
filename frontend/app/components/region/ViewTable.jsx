...
const ViewTable = ({ data, token, teams, title }) => {
  ...
  useEffect(() => {
    if (!searchTerm) {
      ...
      const result = data.data.filter(
        (location) =>
          location.location_name?.toLowerCase().includes(lower) ||
          location.location_code?.toLowerCase().includes(lower) ||
          location.address?.toLowerCase().includes(lower) ||
          location.description?.toLowerCase().includes(lower)
      );
  ...
  const handleAddLocation = async (jsonData) => {
    setIsLoading(true);
    try {
      const res = await fetch(`/frontapi/region`, {
        method: "POST",
        headers: {
  ...
  const handleEditLocation = async (jsonData) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`/frontapi/region?id=${jsonData.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteLocation = async (id) => {
    setIsLoading(true);
    try {
      const res = await fetch(`/frontapi/region?id=${id}`, {
        method: "DELETE",
      });
  ...
  const handleDelete = async (location) => {
    confirmDelete({
      ...
      onSuccess: () => console.log("Location deleted successfully!"),
    });
  };
  ...
  return (
    <div className="border border-stroke bg-white px-6 pt-6 pb-4 rounded-lg shadow-md dark:border-strokedark dark:bg-boxdark">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <PermissionGate permissions={["add-regions"]}>
          <Button
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-100 dark:bg-gray-800 text-left text-sm font-semibold">
              <th className="p-4 rounded-tl-lg">Division</th>
              <th className="p-4 rounded-tl-lg">{title} Details</th>
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900 dark:text-gray-100">
                          {location.team && location.team['name'] || "No Team"}
                        </p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          ID : {location.team && location.team['id'] || "No ID"}
                        </p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <PermissionGate permissions={["edit-regions"]}>
                        <button
                      ...
                      <PermissionGate permissions={["delete-regions"]}>
                        <button
